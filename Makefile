...
build-image-resource:
	cd src/resources/img && pyrcc5 -o ../../woffle/imageresources.py image-resources.qrc

build-font-resource:
	cd src/resources/fonts && pyrcc5 -o ../../woffle/fontresources.py font-resources.qrc
...
build-macos: macos-iconset build-image-resource build-font-resource
	pyinstaller --noconfirm "target/PyInstaller-macOS/Woffle-macOS.spec"
	cp LICENSE dist/license.txt

build-macos-installer:
	# https://github.com/sindresorhus/create-dmg
	- rm dist/*.dmg
	cd dist && create-dmg --identity="BOGUS" Woffle.app
...
codesign-macos:
	codesign --deep --timestamp --force --options runtime -s "Developer ID Application: Christopher Simpkins" dist/Woffle.app

# verify code signature on the distribution bundle
verify-codesign-macos:
	spctl -a -v dist/Woffle.app
...
upload-macos-installer-for-notarize:
	...
	xcrun altool --notarize-app --type osx --primary-bundle-id "org.sourcefoundry.woffle" --username [USERNAME] --password [PASSWORD] --file dist/*.dmg
...
notarize-macos-installer:
	...
	xcrun altool --notarization-info [APPLE APP UUID] --username [USERNAME]
...
win-ico:
	magick convert icons/512.png -alpha on -resize 256x256 \
